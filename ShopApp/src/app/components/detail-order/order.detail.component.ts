      ...
      next: (response: any) => {
        ...
        this.orderResponse.order_details = response.order_details
          .map((order_detail: OrderDetail) => {
            order_detail.product.thumbnail = order_detail.product.thumbnail ? `${environment.apiBaseUrl}/products/images/${order_detail.product.thumbnail}` : 'assets/img/no-image.svg';
